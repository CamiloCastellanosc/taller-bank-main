...
const accounts = [
  {
    "numberAccount": "0012341",
    "typeAccount": "ahorros",
    "passcode": "123451",
    "owner": "Maritza pineda",
    "balance": 600,
  },
  {
    "numberAccount": "0012342",
    "typeAccount": "corriente",
    "passcode": "123452",
    "owner": "Oscar castellanos",
    "balance": 400,
  },
  {
    "numberAccount": "0012343",
    "typeAccount": "ahorros",
    "passcode": "123453",
    "owner": "Viviana chipatecua",
    "balance": 700,
  },
  {
    "numberAccount": "0012344",
    "typeAccount": "corriente",
    "passcode": "123454",
    "owner": "Laura clavijo",
    "balance": 800,
  ...
  if (validAccount) {
    if (type === "insert") {
      ...
      if (validTransaction === true) {
        ...
        receiptMessage.querySelector("#amount-receipt").textContent = `$${receipt.amount.toFixed(2)}`; //$100.00
        receiptMessage.querySelector("#type").textContent = receipt.type;
        receiptMessage.querySelector("#date").textContent = receipt.date;
        receiptMessage.querySelector("#new-balance").textContent = `$${receipt.newBalance.toFixed(2)}`;
    ...
    if (type === "withdraw") {
      ...
      if (validTransaction === true) {
        ...
        receiptMessage.querySelector("#amount-receipt").textContent = `$ ${receipt.amount.toFixed(2)}`;
        receiptMessage.querySelector("#type").textContent = receipt.type;
        receiptMessage.querySelector("#date").textContent = receipt.date;
        receiptMessage.querySelector("#new-balance").textContent = `$ ${receipt.newBalance.toFixed(2)}`;
        d.querySelector(".modal-form").classList.add("d-none")
